...
<head>
	...
	<body>
		...
		$arquivo = 'matricula_individual.xls';

		// Crear la tabla HTML
		$html = '';
		$html .= '<table border="1">';
		$html .= '<tr>';
		$html .= '<td colspan="5">Tabla Lista de matriculados individualmente</tr>';
		$html .= '</tr>';
        ...
        $html .= '<td><b>colegio de procedencia</b></td>';
        $html .= '<td><b>grado</b></td>';
        $html .= '<td><b>dni</b></td>';
        $html .= '<td><b>fecha de nacimiento</b></td>';
        $html .= '<td><b>celular </b></td>';
		$html .= '<td><b>correo</b></td>';
		$html .= '<td><b>ciclo al que se matricula </b></td>';
		$html .= '<td><b>foto</b></td>';
		$html .= '<td><b>fecha de registro</b></td>';
		$html .= '</tr>';

		//Seleccionar todos los elementos de la tabla
		$result_msg_contatos = "SELECT * FROM matricula_individual";
		...
		while ($row_msg_contatos = mysqli_fetch_assoc($resultado_msg_contatos)) {
            ...
            $html .= '<td>' . $row_msg_contatos["procedencia_c"] . '</td>';
            $html .= '<td>' . $row_msg_contatos["grado"] . '</td>';
            $html .= '<td>' . $row_msg_contatos["dni"] . '</td>';
            $html .= '<td>' . $row_msg_contatos["fecha_nacimiento"] . '</td>';
            $html .= '<td>' . $row_msg_contatos["celular"] . '</td>';
			$html .= '<td>' . $row_msg_contatos["correo"] . '</td>';
			$html .= '<td>' . $row_msg_contatos["me_matriculo"] . '</td>';

...
if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
    ...
    if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
        $SqlInsertAlumno = ("INSERT INTO quimica_superior(
            namefull,
            foto,
             videos,
            fechaRegistro
    ...
    closedir($miDir);
    header("Location:../quimica_superior.php?a=1");

  }else{
    header("Location:../quimica_superior.php?errorimg=1");
  }
}
...
if($metodoAction == 2){
   ...
   // $section        = filter_var($_POST['section'], FILTER_SANITIZE_STRING);
//
    $UpdateAlumno    = ("UPDATE quimica_superior
    ...
    if (!empty($_FILES["foto"]["name"])){
        ...
        if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
            ...
            //Muevo la foto a mi directorio.
        if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
            $updateFoto = ("UPDATE quimica_superior SET foto='$nuevoNameFoto' WHERE id='$idAlumno' ");
  ...
  header("Location:../quimica_superior.php?update=1&id=$idAlumno");
 }

...
if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
    ...
    if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
        $SqlInsertAlumno = ("INSERT INTO sobre_sulemath(
    ...
    closedir($miDir);
    header("Location:../sobre_sulemath.php?a=1");

  }else{
    header("Location:../sobre_sulemath.php?errorimg=1");
  }
}
...
if($metodoAction == 2){
   ...
   // $section        = filter_var($_POST['section'], FILTER_SANITIZE_STRING);
//
    $UpdateAlumno    = ("UPDATE sobre_sulemath
        SET videos='$videos'
    ...
    if (!empty($_FILES["foto"]["name"])){
        ...
        if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
            ...
            //Muevo la foto a mi directorio.
        if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
            $updateFoto = ("UPDATE sobre_sulemath SET foto='$nuevoNameFoto' WHERE id='$idAlumno' ");
  ...
  header("Location:../sobre_sulemath.php?update=1&id=$idAlumno");
 }

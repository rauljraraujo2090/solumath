...
if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
    ...
    if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
        $SqlInsertAlumno = ("INSERT INTO sexto_grado(
            namefull,
            foto,
             videos,
            fechaRegistro
    ...
    closedir($miDir);
    header("Location:../6primaria.php?a=1");

  }else{
    header("Location:../6primaria.php?errorimg=1");
  }
}
...
if($metodoAction == 2){
   ...
   // $section        = filter_var($_POST['section'], FILTER_SANITIZE_STRING);
//
    $UpdateAlumno    = ("UPDATE sexto_grado
    ...
    if (!empty($_FILES["foto"]["name"])){
        ...
        if (!((strpos($tipo_foto, "PNG") || strpos($tipo_foto, "jpg") && ($tamano_foto < 100000)))) {
            ...
            //Muevo la foto a mi directorio.
        if(move_uploaded_file($sourceFoto, $urlFotoAlumno)){
            $updateFoto = ("UPDATE sexto_grado SET foto='$nuevoNameFoto' WHERE id='$idAlumno' ");
  ...
  header("Location:../6primaria.php?update=1&id=$idAlumno");
 }
